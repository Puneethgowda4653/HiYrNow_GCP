...
export class UserProfileShareComponent implements OnInit, OnChanges {
  @Input() feature?: string;
  @Input() userId?: string; // Allow userId to be passed as input
  ...
  private readonly footerText =
    'Resume generated by HiyrNow and verified by HiyrNow';

  @ViewChild(ContactDialogComponent)
  ...
  private async addBrandingToPDF(pdf: jsPDF, pageCount: number) {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      pdf.text(this.footerText, pageWidth / 2, pageHeight - 5, {
        align: 'center',
      });
  ...
  ngOnInit(): void {
    // If userId is provided as input, use it directly
    if (this.userId) {
      this.userIdFromRoute = this.userId;
      this.fetchUserDetails();
      this.loadProfilePic(this.userIdFromRoute);
      this.incrementViewCount();
      return;
    }

    // Otherwise, get userId from route params (existing behavior)
    this.route.paramMap.subscribe((params) => {
      ...
      if (this.userIdFromRoute) {
        ...
        this.incrementViewCount();
        if (this.jobId && this.userIdFromRoute) {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // Watch for userId input changes
    if (changes['userId'] && !changes['userId'].firstChange) {
      const newUserId = changes['userId'].currentValue;
      if (newUserId && newUserId !== this.userIdFromRoute) {
        this.userIdFromRoute = newUserId;
        this.fetchUserDetails();
        this.loadProfilePic(this.userIdFromRoute);
      }
    } else if (changes['userId'] && changes['userId'].firstChange && this.userId) {
      // If userId is provided on first change, use it
      this.userIdFromRoute = this.userId;
      this.fetchUserDetails();
      this.loadProfilePic(this.userIdFromRoute);
    }
  ...
  getJobApplication(): void {
    this.jobPosting
      .getJobApplicationByJobIdAndUserId(this.jobId, this.userIdFromRoute)
      .subscribe(
        (data) => {
  ...
  fetchUserDetails(): void {
    this.userService.getUserResume(this.userIdFromRoute).subscribe(
      (response: any[]) => {
    ...
    this.userService.getUserDetails(this.userIdFromRoute, this.feature).subscribe(
      (response: any) => {
        if (response.status === 'success' && response.data) {
          ...
          if (this.user?.firstName && this.user?.lastName) {
            ...
            this.meta.updateTag({
              name: 'description',
              content: `View ${fullName}'s professional profile on HiyrNow`,
            });
            this.meta.updateTag({
              property: 'og:title',
              content: `${fullName} - Professional Profile`,
            });
            this.meta.updateTag({
  ...
  downloadPdf(filename: string, contentType: string) {
    this.userService.downloadPDF(filename, this.userIdFromRoute).subscribe(
      (res: Blob) => {
  ...
  getTopSkills() {
    return (
      this.userData?.skill
        ?.sort(
          (a: any, b: any) => parseInt(b.skillLevel) - parseInt(a.skillLevel)
        )
        .slice(0, 5) || []
    );
  }
  ...
  calculateDuration(exp: any): string {
    const start = new Date(
      exp.startDate.year,
      this.getMonthNumber(exp.startDate.month)
    );
    const end =
      exp.ongoingStatus === 'true'
        ? new Date()
  ...
  getProjectCount(skillName: string): number {
    return (
      this.userData.project?.filter((proj: any) =>
  ...
  viewResume() {
    if (this.userData?.user?.resume) {
      this.downloadPdf(this.userData.user.resume, 'application/pdf');
    } else {
      // Show a message that no resume is available
